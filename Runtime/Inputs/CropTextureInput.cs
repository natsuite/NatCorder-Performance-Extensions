...
namespace NatSuite.Recorders.Inputs {
    ...
    public sealed class CropTextureInput : ITextureInput { // INCOMPLETE
        ...
        public CropTextureInput (ITextureInput input) {
            ...
            this.cropRect = new RectInt(0, 0, input.frameSize.width, input.frameSize.height);
            this.aspectMode = 0;
        }
        ...
        private void CommitFrame (Texture source, RenderTexture destination) {
            ...
            var cropOffs = float2(cropRect.xMin, cropRect.yMin) / float2(width, height);
            var cropSize = float2(cropRect.width, cropRect.height) / float2(width, height);
            var A = float3x3(
                1f, 0f, 1f,
                0f, 1f, 1f,
                1f, 1f, 1f
